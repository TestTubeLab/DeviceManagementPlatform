...
# FRP 配置
FRP_CONFIG_FILE = "/etc/frp/frpc.ini"  # frpc 配置文件路径
...
def register_device():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 🔥 处理 FRP 配置（如果服务端返回了）
            frp_config = result.get('frp_config')
            if frp_config:
                logger.info("收到 FRP 配置，开始应用...")
                success, error = apply_frp_config(frp_config)
                if success:
                    logger.info("FRP 配置应用成功")
...
# ==================== FRP 内网穿透管理 ====================
def get_local_frp_config_version():
    """获取本地 FRP 配置版本"""
...
def save_local_frp_config_version(version):
    """保存本地 FRP 配置版本"""
    version_file = "/etc/frp/.version"
    try:
        ...
        logger.warning(f"保存 FRP 配置版本失败: {e}")


def render_frpc_ini(frp_config):
    """
    根据云端配置生成 frpc.ini 内容
    frp_config: {
        'server_addr': 'frp9.mmszxc.xin',
        'server_port': 39981,
        'token': '198631',
        'config_version': 1,
        'tunnels': {
    ...
    lines = [
        "# FRP Client Configuration - Auto Generated by Device Agent",
        "# DO NOT EDIT MANUALLY",
        "",
        "[common]",
        f"server_addr = {frp_config['server_addr']}",
        f"server_port = {frp_config['server_port']}",
        f"token = {frp_config['token']}",
        "",
    ]
    
    # 添加隧道配置
    tunnels = frp_config.get('tunnels', {})
    for tunnel_name, tunnel_cfg in tunnels.items():
        lines.append(f"[{device_id}_{tunnel_name}]")
        lines.append(f"type = {tunnel_cfg.get('type', 'tcp')}")
        lines.append(f"local_ip = 127.0.0.1")
        lines.append(f"local_port = {tunnel_cfg['local_port']}")
        lines.append(f"remote_port = {tunnel_cfg['remote_port']}")
        lines.append("")
...
def ensure_frpc_service():
    """确保 frpc systemd 服务存在"""
    service_file = f"/etc/systemd/system/{FRP_SERVICE_NAME}.service"
...
[Service]
Type=simple
ExecStart={FRP_BINARY_PATH} -c {FRP_CONFIG_FILE}
Restart=always
RestartSec=5
...
"""
    ...
    try:
        ...
        subprocess.run(["systemctl", "enable", FRP_SERVICE_NAME], check=True, timeout=30)
        logger.info(f"frpc 服务已创建: {service_file}")
        return True
        
    except Exception as e:
        logger.error(f"创建 frpc 服务失败: {e}")
        return False


def apply_frp_config(frp_config):
    """
    应用 FRP 配置：生成配置文件，启动/重启服务
    返回: (success, error_message)
    """
    try:
        # 1. 确保 frpc 二进制存在
        if not ensure_frpc_binary():
            return False, "frpc 二进制不存在且下载失败"
        
        # 2. 生成配置文件
        config_dir = os.path.dirname(FRP_CONFIG_FILE)
        os.makedirs(config_dir, exist_ok=True)
        
        ini_content = render_frpc_ini(frp_config)
        with open(FRP_CONFIG_FILE, 'w') as f:
            f.write(ini_content)
        logger.info(f"FRP 配置已写入: {FRP_CONFIG_FILE}")
        
        # 3. 确保 systemd 服务存在
        if not ensure_frpc_service():
            return False, "创建 frpc 服务失败"
        
        # 4. 重启服务
        subprocess.run(["systemctl", "restart", FRP_SERVICE_NAME], check=True, timeout=30)
        logger.info("frpc 服务已重启")
        
        # 5. 等待并检查状态
        time.sleep(3)
        result = subprocess.run(
            ["systemctl", "is-active", FRP_SERVICE_NAME],
            capture_output=True, text=True
        )
        
        if result.stdout.strip() == "active":
            # 保存配置版本
            config_version = frp_config.get('config_version', 1)
            save_local_frp_config_version(config_version)
            logger.info(f"FRP 配置应用成功，版本: {config_version}")
            return True, ""
        else:
            return False, f"frpc 服务未能正常启动: {result.stdout.strip()}"
...
def report_frp_status(status, error_message=""):
    """上报 FRP 状态到云端"""
    device_id = get_device_id()
    try:
        requests.post(
            f"{CLOUD_SERVER}/devices/{device_id}/report_frp_status/",
            json={"status": status, "error_message": error_message},
            timeout=10
        )
    except Exception as e:
        logger.debug(f"上报 FRP 状态失败: {e}")
...
def execute_project_deployment(deployment):
    ...
    try:
        ...
        # ========== 宿主机钩子 (post-deploy) ==========
        post_deploy_script = os.path.join(code_mount_path, "MiddlewareServer", "setup-browser.sh")
        if os.path.isfile(post_deploy_script):
            ...
            if hook_result.returncode == 0:
                ...
                if hook_result.stdout:
                    logger.info(f"钩子输出: {hook_result.stdout}")
            else:
    ...
    while True:
        try:
            ...
            if current_time - last_heartbeat >= HEARTBEAT_INTERVAL:
                ...
                # 🔥 处理 FRP 配置更新
                if command.get("frp_update_required"):
                    frp_config = command.get("frp_config")
                    if frp_config:
                        logger.info("检测到 FRP 配置更新，开始应用...")
                        success, error = apply_frp_config(frp_config)
                        if success:
                            logger.info("FRP 配置更新成功")
                            report_frp_status("connected")
                        else:
